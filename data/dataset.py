...
import numpy as np
import os
from utils.utils import compute_ptcloud_from_depth_np

class StereoDataset(torch.utils.data.Dataset):
    def __init__(self, opt):
        self.opt = opt
        self.scene_names = sorted([d for d in os.listdir(opt.dataset_root) if os.path.isdir(os.path.join(opt.dataset_root, d))])
